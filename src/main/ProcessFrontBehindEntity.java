			...
			if(titleNum != 0) {
				if(x >= -2*gp.tileSize && x <= gp.screenWidth + 2*gp.tileSize &&
					...
					for(Entity e: entities) {
						if(e.worldX + e.solidAreaDefaultX/2 >= worldX - 25 && e.worldX + e.solidAreaDefaultX/2 <= worldX + 25 + layer.tiles[titleNum].getWidth() && e.worldY >= worldY && e.worldY <= (worldY + layer.tiles[titleNum].collisionArea.y + layer.tiles[titleNum].collisionArea.height) &&
